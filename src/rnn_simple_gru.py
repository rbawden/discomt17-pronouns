...
import argparse
from keras.layers import Dense, Dropout, Activation, Merge, Input, merge, concatenate
...
if __name__=="__main__":
    ...
    # load data
    #---------------------------------------------------------------------------
    vs = read_vocabularies(tr_dt_file, force_rebuild = args.force_rebuild)
    
    #vs.trainable = False Doesn't work :/
    ...
    print("raw dev")
    dev_data = fill_batch(dev_ms, vs, raw_dev_data).__next__()
    print("filled batch")
    ...
    # extra embeddings for extra inputs
    extra_embeddings = []
    extra_vectors = []
    for e in range(len(vs.extra)):
    ...
    if stacked:
        ...
        merged_vector = concatenate([right_target_wordpos_lstm_out,
                               ...
                               right_source_lstm_out]+ extra_out+
                               [vector_pronoun] ,
                               axis =-1)
    ...
    model = Model(inputs=[left_target_wordpos,
                         right_target_wordpos,
                         left_target, right_target,
                         left_target_pos,
                         right_target_pos,
                         left_source,
                         right_source] + extra_inputs +
                         [pronoun_input],
                         outputs=predictions)
